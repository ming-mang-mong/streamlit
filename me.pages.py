...
def analysis_page():
    ...
    with tab1:
        tit = st.selectbox(
            '당신의 직업을 선택하세요.',
            ('학생', '직장인', '프로게이머', '크리에이터', '기타'), key="job_select_tab1")
        if tit == '학생':
    ...
    with tab2:
        tit = st.selectbox(
            '당신의 직업을 선택하세요.',
            ('학생', '직장인', '프로게이머', '크리에이터', '기타'), key="job_select_tab2")
        if tit == '학생':
            ...
            st.caption('NuPhy Field75HE')
            st.image('https://encrypted-tbn0.gstatic.com/shopping?q=tbn:ANd9GcRTLXP5HgsxW4sBtBaINVMofjakxRHJENGJlm-9MDhMSSe51yarcXpcFaCt0LHg64kIUFux9gj-CeMgHZZVS64sAcaXB0FuUvEIi2W7OMJU7Gmhj0dNNKfTlP8')
        elif tit == '크리에이터':
            st.caption('Keychron Q1 HE')
            st.image('https://encrypted-tbn1.gstatic.com/shopping?q=tbn:ANd9GcQ_dniZqQgIfEmng7unq0HpJmuY8zbXRykXQdCHrkheyAa5qqCqOI-iAMaN4v6SboAIDXYDnKm0AJA6OoJDxnX03D2kINHRob9pvZV9HFRJ9qU4cqg739htpT0')
        else:
            st.caption('Keychron C1 Pro 8K')
            st.image('https://img.danawa.com/prod_img/500000/453/055/img/28055453_1.jpg?_v=20230906110418&shrink=360:360')
    ...
    with tab3:
        tit = st.selectbox(
            '당신의 직업을 선택하세요.',
            ('학생', '직장인', '프로게이머', '크리에이터', '기타'), key="job_select_tab3")
        if tit == '학생':
